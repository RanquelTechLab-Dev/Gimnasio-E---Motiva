...
export type DeleteStudentPreview = {
  ok: boolean
...
export type DeletePaymentPreview = {
  ...
  affected: {
    future_active_bookings_to_cancel: number
  }
  warnings: string[]
  details: {
    payment_id: string
    ...
    approved_paid_total: number
    approved_without_payment: number
    confirmation_required: string
  }
}

export type DeleteClassSessionPreview = {
  ok: boolean
...
export type DefinitiveDeleteResult = {
  ok: boolean
  action: string
  student_id?: string
  payment_id?: string
