-- RAN-34: fix student file visibility defaults and fixed schedule combo preview.
--
-- Safety:
-- - No operational rows are removed by this migration.
-- - No bookings are created by preview.
-- - Bulk booking behavior remains explicit through the existing admin action.
-- - The combo backfill only links missing plan activities for an existing plan.
...
join public.activities a
  on a.slug in ('semi_personalizado', 'neurofuncional')
where p.slug = 'combo_semipersonalizado_y_funcional'
  and not exists (
    select 1
...
drop function if exists public.admin_preview_fixed_schedule_for_student(
  uuid,
  uuid,
  integer[],
  time
...
drop function if exists public.admin_bulk_book_fixed_schedule_for_student(
  uuid,
  uuid,
  integer[],
  time
...
drop function if exists private.admin_fixed_schedule_summary(
  uuid,
  uuid,
  integer[],
  time,
...
begin
  ...
  if p_activity_id is null then
    raise exception 'Actividad requerida.';
  end if;
  ...
  if not exists (
    ...
    raise exception 'La actividad no pertenece al programa o esta inactiva.';
  end if;
  ...
  for v_candidate in
    select
      ...
      coalesce(
        ...
        pa.monthly_credits,
        p.package_class_count
      ) as weekly_class_limit,
      case
    ...
    where pa.plan_id = v_membership.plan_id
      ...
      and (s.starts_at at time zone 'America/Argentina/Buenos_Aires')::date
        between v_membership.start_date and v_membership.end_date
      and extract(
        isodow
        from s.starts_at at time zone 'America/Argentina/Buenos_Aires'
      )::integer = any(v_weekdays)
      and (s.starts_at at time zone 'America/Argentina/Buenos_Aires')::time
        = p_start_time
  ...
  loop
    ...
    if v_candidate.session_active is not true or v_candidate.cancelled_at is not null then
      ...
      v_planned_for_week :=
        coalesce((v_weekly_planned ->> v_week_key)::integer, 0);

      if coalesce(v_candidate.weekly_classes_used, 0) + v_planned_for_week
        >= v_candidate.weekly_class_limit then
    ...
    if p_execute and v_status = 'available' then
      ...
      if v_existing.id is not null
        and v_existing.status = 'cancelled'::public.booking_status then
  ...
  return jsonb_build_object(
    ...
    'available_count', case
      when p_execute then v_created_count
      else v_available_count
    end,
    ...
    (
      select jsonb_agg(
        jsonb_build_object(
          ...
          'label', item.start_time || ' · ' || item.activity_name,
          'activity_names', item.activity_name,
          'sessions_count', item.sessions_count
        )
        order by item.start_time, item.activity_name
      )
      from (
        select
          s.activity_id,
          a.name as activity_name,
          left(
            ((s.starts_at at time zone 'America/Argentina/Buenos_Aires')::time)::text,
            5
          ) as start_time,
        ...
        group by
          s.activity_id,
          a.name,
          (s.starts_at at time zone 'America/Argentina/Buenos_Aires')::time
      ) item
    ),
    '[]'::jsonb
  );
...
begin
  ...
  return private.admin_fixed_schedule_summary(
    p_student_id,
    p_membership_id,
    v_activity_id,
    p_weekdays,
...
begin
  ...
  return public.admin_bulk_book_fixed_schedule_for_student(
    p_student_id,
    p_membership_id,
    v_activity_id,
    p_weekdays,
...
revoke all on function private.admin_fixed_schedule_summary(
  uuid,
  uuid,
  uuid,
  integer[],
...
revoke all on function public.admin_list_fixed_schedule_options_for_student(
  uuid,
  uuid
) from public, anon;

revoke all on function public.admin_preview_fixed_schedule_for_student(
  uuid,
  uuid,
  integer[],
  time
...
grant execute on function public.admin_list_fixed_schedule_options_for_student(
  uuid,
  uuid
) to authenticated;

grant execute on function public.admin_preview_fixed_schedule_for_student(
  uuid,
  uuid,
  integer[],
  time
